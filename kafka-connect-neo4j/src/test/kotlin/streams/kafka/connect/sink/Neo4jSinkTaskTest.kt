...
class Neo4jSinkTaskTest: EasyMockSupport() {
    ...
    fun `should insert data into Neo4j`() = runBlocking {
        ...
        db.graph().beginTx().use {
            ...
            assertEquals(0, personCount.toInt()) // data not insert because the batch size is not achieved and the timeout has not arrived
    ...
    @Test
    fun `should not insert data into Neo4j`() = runBlocking {
        val topic = "neotopic"
        val props = mutableMapOf<String, String>()
        props[Neo4jSinkConnectorConfig.SERVER_URI] = db.boltURI().toString()
    ...
    @Test
    fun `should write data with batch size`() = runBlocking {
        val firstTopic = "neotopic"
        val props = mutableMapOf<String, String>()
        props[Neo4jSinkConnectorConfig.SERVER_URI] = db.boltURI().toString()
        props["${Neo4jSinkConnectorConfig.TOPIC_CYPHER_PREFIX}$firstTopic"] = "CREATE (n:Person {name: event.firstName, surname: event.lastName})"
        props[Neo4jSinkConnectorConfig.AUTHENTICATION_TYPE] = AuthenticationType.NONE.toString()
        props[Neo4jSinkConnectorConfig.BATCH_SIZE] = 5.toString()
        ...
        task.initialize(mock<SinkTaskContext, SinkTaskContext>(SinkTaskContext::class.java))
        repeat(5) {
            val input = listOf(SinkRecord(firstTopic, 1, null, null, PERSON_SCHEMA, struct, it.toLong()))
            task.put(input)
            if (it < 4) { // test the data written
                delay(1000)
                db.graph().beginTx().use {
                    val personCount = db.graph().execute("MATCH (p:Person) RETURN COUNT(p) as COUNT").columnAs<Long>("COUNT").next()
                    assertEquals(0, personCount.toInt())
                }
            }
        ...
        db.graph().beginTx().use {
            val personCount = db.graph().execute("MATCH (p:Person) RETURN COUNT(p) as COUNT").columnAs<Long>("COUNT").next()
            assertEquals(5, personCount.toInt())
        }
        task.stop()
    }
}