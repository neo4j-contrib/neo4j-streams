...
[NOTE]
====
If you choose to install the plugin in this way, please note that the connector will work because the `CONNECT_PLUGIN_PATH` environment variable already contains the default path (`/usr/share/confluent-hub-components`) where the `confluent-hub` tool installed the plugin.
Having that said, if you want to install it under a different path, you have to add that path to the above environment variable.
====
...
To create the Sink instance and configure your preferred ingestion strategy, you can follow instructions described
into xref:kafka-connect.adoc#kafka-connect-source-instance[Create the Source Instance].
sections.
