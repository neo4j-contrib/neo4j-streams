...
With `kafka.log.cleanup.policy=delete` will be generated a sequence of unique keys with Neo4j Streams Source.
instead with `kafka.log.cleanup.policy=compact` the keys will be adapted to enable
