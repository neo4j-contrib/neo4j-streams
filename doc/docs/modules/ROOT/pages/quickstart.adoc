...
list of options available, see the documentation subsections on the xref:producer.adoc#neo4j_streams_source[source] and xref:consumer.adoc#neo4j_streams_sink[sink].
...
. _Optional_: Configuring Neo4j to produce records to Kafka (xref:producer.adoc#neo4j_streams_source[Source])
...
For full details on what you can do here, see the xref:producer.adoc#neo4j_streams_source[Source] section of the documentation.
...
2020-03-25 20:13:50.608+0000 WARN  Unrecognized setting. No declared setting with name: kafka.zookeeper.connect
...
Download and install the plugin via Confluent Hub client. See the chapter xref:kafka-connect.adoc#kafka_connect_plugin[Kafka Connect Plugin] for more details.
