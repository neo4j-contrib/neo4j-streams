...
----
"{environment}.topic.cypher.my-topic": "MERGE (p:Person{name: event.name, surname: event.surname, ts: event.timestamp, tc: event.__headers__.timeCommit, dbId: event.__key__.id}) MERGE (f:Family{name: event.surname}) MERGE (p)-[:BELONGS_TO]->(f)"
----
...
[source,cypher]
----
MERGE (p:Person{name: event.name, surname: event.surname, ts: event.timestamp, tc: event.__headers__.timeCommit, dbId: event.__key__.id})
...
Where `{batch}` is a list of event objects, holding at least these three objects:

* **`$$__timestamp__$$`:** Records' timestamp as `Long`. The `$$__timestamp__$$` prefix can be customized using the `neo4j.event.timestamp` property.
* **`$$__headers__$$`:** Records' headers as an Object, so you can access fields from the headers using `$$event.__headers__.field$$`. The `$$__headers__$$` prefix can be customized using the `neo4j.event.headers` property.
* **`$$__key__$$`:** Records' key as an Object, , so you can access fields from the headers using `$$event.__headers__.field$$`. The `$$__key__$$` prefix can be customized using the `neo4j.event.key` property.
