...
Where `{batch}` is a json list with at least three fields related to timestamp, headers (every value on a header will be treated as a String) and key, so continuing with the example above a possible full representation could be:

[source,cypher]
----
:params events => [
  {
    "__timestamp__": 1590071463,
    "__headers__": {
      "timeCommit": 1590071463,
      "source": "Raspberry Pi"
    },
    "__key__": {
      "dbTransaction": "55x88at7f"
    },
...
MERGE (n:Label {id: event.id})
    ON CREATE SET n += event.properties,
                    n.ts = event.__timestamp__
                    n.source = event.__headers__.source,
                    n.trxId = event.__key__.dbTransaction
----
