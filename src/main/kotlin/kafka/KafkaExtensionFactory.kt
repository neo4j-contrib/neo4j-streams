        ...
        return object : LifecycleAdapter() {
            override fun start() {
                ...
                try {
                    ...
                    AdminClient.create(config.asProperties()).use { it.createTopics(listOf(NewTopic(config.topic, config.partitionSize, config.replication.toShort()))) }

                    val producer = KafkaProducer<Long, ByteArray>(config.asProperties())
